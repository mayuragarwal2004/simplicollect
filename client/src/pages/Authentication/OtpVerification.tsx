...
const OtpVerification: React.FC = () => {
  ...
  const inputType = locationState?.inputType;
  const selectedCountry = locationState?.selectedCountry || 'IN';
  ...
  const [isPhoneParsed, setIsPhoneParsed] = useState<boolean>(false);
  ...
  useEffect(() => {
    if (!locationState?.identifier) {
      navigate('/auth/signin', { state: { from: locationState?.from } });
    } else if (locationState?.identifier) {
      if (inputType === 'phone') {
        try {
          parsePhoneNumberWithError(locationState.identifier);
        } catch (error) {
  ...
  }, [locationState, navigate, inputType]);
  ...
  const handleIdentifierChange = (e: React.ChangeEvent<HTMLInputElement>): void => {
              ...
              <div className="mb-4">
                ...
                {inputType === 'phone' ? (
                  <PhoneInput
                    ...
                    disabled
                    inputComponent={React.forwardRef<HTMLInputElement, any>((props, ref) => (
                      <Input
                        {...props}
                        ref={ref}
                        disabled
