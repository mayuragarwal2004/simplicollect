...
import { ReportAPagination } from './reporta-pagination.jsx'

export function ReportATable({
 columns,
 data,
 searchInputField,
 filterField,
 ...
 const updateUrlQuery = () => {
  ...
  const queryString = createQueryString({ limit, skip }, searchParams)

  navigate(`?${queryString}`, { replace: true })
 }

 // Trigger `updateUrlQuery` whenever `limit` or `skip` changes
 React.useEffect(() => {
  updateUrlQuery()
 }, [limit, skip])
 ...
 return (
  <div className="space-y-4">
   ...
   <ReportAPagination table={table} totalRecord={totalRecord} />
  </div>
 )
}
