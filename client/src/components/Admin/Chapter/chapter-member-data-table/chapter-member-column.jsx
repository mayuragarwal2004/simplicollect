...
export const MemberColumn = (allRoles) => [
  ...
  },
  {
    accessorKey: 'role',
    header: () => <div>Role</div>,
    cell: ({ row }) => <p className="text-sm">{row.original.roleNames}</p>,
    enableSorting: false,
    enableHiding: false,
  },
  {
  ...
  },
  {
    ...
    cell: ({ row }) => {
      ...
      const handleChangeRole = () => {
        axiosInstance
          .put(
            `/api/admin/chapter-member-list/${chapterSlug}/members/${row.original.memberId}/role`,
            {
      ...
      const handleUpdateBalance = () => {
        axiosInstance
          .put(
            `/api/admin/chapter-member-list/${chapterSlug}/members/${row.original.memberId}/balance`,
            {
              balance,
              addToTransaction,
            },
      ...
      const handleRemove = () => {
        axiosInstance
          .put(
            `/api/admin/chapter-member-list/${chapterSlug}/members/${row.original.memberId}/remove`,
            {
              leaveDate,
            },
          )
      ...
      const handleDelete = () => {
        axiosInstance
          .delete(
            `/api/admin/chapter-member-list/${chapterSlug}/members/${row.original.memberId}`,
          )
          .then(() => {
      ...
      const handleRolesChange = (selectedValues) => {
        ...
        <div className="flex flex-wrap gap-2">
          ...
          <Dialog open={isRemoveDialogOpen} onOpenChange={setRemoveDialogOpen}>
            ...
            <DialogContent>
              ...
              <DialogDescription>
                Enter leave date to remove this member.
              </DialogDescription>
              <Input
                type="date"
                value={leaveDate}
