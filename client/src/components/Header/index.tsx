...
const Header = (props: {
  ...
  const { memberData, chapterData, allChaptersData, switchChapter } = useData();
  ...
  return (
    <header className="sticky top-0 z-[10] flex w-full bg-white drop-shadow-1 dark:bg-boxdark dark:drop-shadow-none">
      <div className="flex flex-grow items-center justify-between px-4 py-4 shadow-2 md:px-6 2xl:px-11">
        ...
        {/* choose chapter */}
        <div>
          {allChaptersData?.length > 1 && (
            <select
              className="hidden lg:block w-48 h-10 px-4 py-2 bg-white border border-stroke rounded-md shadow-sm dark:bg-boxdark dark:border-strokedark dark:text-white"
              onChange={(e) => {
